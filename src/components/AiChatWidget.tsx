...
export function AiChatWidget() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (body.usage) setUsageLabel(`${body.usage.count}/${body.usage.limit} vandaag`);
    } catch {
  ...
  return (
    <aside className="ai-chat" aria-label="Jaakie kennisbankchat">
      {open ? (
        <section className="ai-chat-panel" aria-live="polite">
          ...
          <form className="ai-chat-form" onSubmit={handleSubmit}>
            <textarea
              aria-label="Vraag voor Jaakie"
              maxLength={1000}
              onChange={(event) => setInput(event.target.value)}
              placeholder="Vraag over Jaakie of PC 200"
              rows={2}
              value={input}
            />
        ...
        <button className="ai-chat-launcher" type="button" onClick={() => setOpen(true)} aria-label="Open Jaakie chat">
