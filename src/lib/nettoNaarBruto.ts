...
function forwardNetto(brutoloon: number, input: NettoNaarBrutoInput): NettoResultaat {
...
export function zoekBrutoVoorNetto(input: NettoNaarBrutoInput): NettoNaarBrutoResultaat {
    ...
    const tolerantie = (tolerantieDatapunt?.waarde_genormaliseerd as number | undefined) ??
        DEFAULT_TOLERANTIE;
    ...
    while (eurNaarCent(forwardNetto(bovengrensCent / 100, input).nettoloon) < doelCent &&
