...
export function valideerLoonrunInput(
  inputs: Array<{ id: string; naam: string; profiel: Profiel }>,
): LoonrunValidatie[] {
  ...
  voegContextBlokkeringToe(
    meldingen,
    contexten,
    "ondernemingsnummer",
    "gemengd_ondernemingsnummer",
    "De loonrun bevat meerdere ondernemingsnummers.",
  );
...
function voegContextBlokkeringToe<K extends "periode" | "refDatum" | "werkgever" | "ondernemingsnummer">(
  meldingen: LoonrunValidatie[],
  contexten: Array<Record<K, string>>,
  key: K,
  code: string,
  boodschap: string,
): void {
