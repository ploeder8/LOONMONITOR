...
function releaseGateMancoVoorActiefDatapunt(datapunt: Datapunt): string[] {
  const mancos: string[] = [];
  if (!heeftTekst(datapunt.id)) mancos.push("id");
  if (datapunt.status !== "actief") mancos.push("status");
  if (!heeftTekst(datapunt.bron_url)) mancos.push("bron_url");
  if (!heeftTekst(datapunt.bron_organisatie)) mancos.push("bron_organisatie");
  if (!heeftTekst(datapunt.geldig_vanaf)) mancos.push("geldig_vanaf");
  if (!heeftTekst(datapunt.bron_fragment)) mancos.push("bron_fragment");
  if (!heeftTekst(datapunt.betrouwbaarheid)) mancos.push("betrouwbaarheid.tier");
  return mancos;
}
...
describe("dataset release-gate", () => {
  it("elk actief datapunt draagt volledige bronmetadata voor audit en release", () => {
    const mancos = alleDatapunten()
      ...
      .flatMap((datapunt) =>
        releaseGateMancoVoorActiefDatapunt(datapunt).map((veld) => `${datapunt.id}: ${veld}`)
      );
  ...
  it("dataset-aantallen in meta, README, AGENTS, DATASET_REFERENCE en ScopePage blijven synchroon", () => {
    ...
    const datasetReference = readFileSync(
      new URL("../../../knowledgebase/DATASET_REFERENCE.md", import.meta.url),
      "utf8"
    );
