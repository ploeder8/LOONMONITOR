...
import { berekenNettoVoorProfiel, berekenWerkgeverskostVoorProfiel, berekenMobiliteitVoorProfiel, berekenVaaWerkmiddelenVoorProfiel, } from "@/lib/profielBerekeningen";
...
function assertRegelBestaat(regels: LoonficheRegel[], code: string): LoonficheRegel {
    ...
    if (!regel)
        throw new Error(`Regel ${code} niet gevonden`);
    return regel;
}
...
describe("Loonfiche — netto naar bruto", () => {
    ...
    it("werkgeverskost gebruikt het berekende bruto, niet DEFAULTS brutoloon", () => {
        ...
        const wgkMetEffectiefBruto = berekenWerkgeverskostVoorProfiel(profiel, refDatum, vaaWerkmiddelen, mobiliteit, r1000.bedrag);
