...
describe("Loonrun — domeinlaag", () => {
    it("berekent 3 geldige werknemers correct", () => {
        ...
        const brutoSom = run.werknemers.reduce((sum, w) => sum + (w.loonfiche?.totalen.brutoRszBasis ?? 0), 0);
    ...
    it("totalen zijn som van individuele loonfiches", () => {
        ...
        const run = bouwLoonrun(inputs);
        const nettoSom = run.werknemers.reduce((sum, w) => sum + (w.loonfiche?.totalen.nettoTeBetalen ?? 0), 0);
        const wgkSom = run.werknemers.reduce((sum, w) => sum + (w.loonfiche?.totalen.werkgeverskostMaand ?? 0), 0);
