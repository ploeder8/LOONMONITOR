...
export type BbszScenario = "individuele_aanslag" | "gemeenschappelijke_aanslag_partner_met_beroepsinkomsten" | "gemeenschappelijke_aanslag_partner_zonder_beroepsinkomsten";
...
export const BBSZ_SCENARIO_LABELS: Record<BbszScenario, string> = {
    ...
    gemeenschappelijke_aanslag_partner_met_beroepsinkomsten: "Gemeenschappelijke aanslag - partner met beroepsinkomsten",
    gemeenschappelijke_aanslag_partner_zonder_beroepsinkomsten: "Gemeenschappelijke aanslag - partner zonder beroepsinkomsten",
};
...
export function bbsz(input: BbszInput): BbszResultaat {
    ...
    if (!dp)
        throw new Error("Datapunt bv_bbsz_schijven_2026 niet gevonden");
...
function berekenKwartaalbijdrage(brutoloon: number, kwartaalloon: number, scenario: BbszScenario): number {
...
function bbszGemeenschappelijkPartnerMetInkomen(brutoloon: number, q: number): number {
    ...
    return Math.min(MAX_KWARTAAL_PARTNER_MET_INKOMEN, round2(43.32 + 0.011 * (brutoloon - 2190.18)));
}
