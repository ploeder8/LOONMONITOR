...
export function bouwLoonrun(inputs: LoonrunWerknemerInput[]): Loonrun {
  ...
  for (const input of inputs) {
    try {
      ...
      aantalBerekend += 1;
      if (status === "concept") aantalConcept += 1;
      if (status === "te_controleren") aantalTeControleren += 1;
      if (status === "gecontroleerd") aantalGecontroleerd += 1;
      if (status === "vastgezet") aantalVastgezet += 1;
    } catch (error) {
