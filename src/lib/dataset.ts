...
// Cast — runtime validation in main.tsx guarantees the shape before we get here.
...
export function indexById(ds: Dataset = dataset): Record<string, Datapunt> {
  if (cachedIndex && ds === dataset) return cachedIndex;
  ...
  if (ds === dataset) cachedIndex = idx;
  return idx;
}

export function getDatapunt(
  datapuntId: string,
  ds: Dataset = dataset,
): Datapunt | null {
...
export function allDatapunten(ds: Dataset = dataset): Datapunt[] {
  const out: Datapunt[] = [];
  for (const cat of DATAPUNT_CATEGORIEEN) out.push(...ds[cat]);
  return out;
}
