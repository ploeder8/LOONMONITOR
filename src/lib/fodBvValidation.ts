...
export type FodBvRootCause = "rsz" | "werkbonus" | "bv" | "bbsz" | "afronding" | "werkgeverskost";
...
const CASE_IDS = Array.from({ length: 30 }, (_, index) => `TC-${String(index + 1).padStart(3, "0")}`);
...
export function summarizeValidation(cases: FodBvValidatieCase[]): FodBvValidatieSummary {
    return cases.reduce<FodBvValidatieSummary>((summary, item) => {
        if (item.status === "ok")
            summary.ok += 1;
        if (item.status === "afwijking")
            summary.afwijking += 1;
        return summary;
    }, { ok: 0, afwijking: 0 });
}
