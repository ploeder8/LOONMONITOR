...
export function berekenWoonwerkVerkeer(input: WoonwerkVerkeerInput): WoonwerkVerkeerResultaat {
    ...
    if (input.trein.actief) {
        componenten.trein = berekenTabelComponent("Trein", "pc200_woonwerk_trein_tabel_2026", input.trein.kmEnkel ?? 0, TREIN_TABEL_2026, input);
    }
...
function berekenPrivewagen(input: WoonwerkVerkeerInput, waarschuwingen: string[]): WoonwerkComponentResultaat {
...
function berekenTabelComponent(label: string, datapuntId: string, kmInput: number, tabel: KmRij[], input: WoonwerkVerkeerInput): WoonwerkComponentResultaat {
