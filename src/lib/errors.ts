// Exception hierarchy — DEVELOPER §5
// Subclassing TypeError/RangeError isn't useful here; we use named classes
// the UI can switch on with `instanceof`.
...
export class DatapuntNietGeldigOpDatum extends PC200DatasetError {
  ...
  constructor(
    ...
    super(
      `${datapuntId} niet geldig op ${refDatum} (geldig: ${geldigVanaf ?? "—"} → ${geldigTot ?? "open"})`,
      datapuntId,
    );
