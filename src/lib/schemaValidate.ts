// Minimal JSON Schema (Draft-07) validator — sufficient for
// pc200_payroll_dataset.schema.json. On a real machine `ajv` would be used;
// the sandbox blocks `npm install`, so this is hand-rolled but covers all
// keywords actually in use by our schema.
//
// Supported keywords:
//   type, required, enum, const, pattern, format (date, uri),
//   minimum, maximum, exclusiveMinimum, minLength, maxLength,
//   items, additionalProperties, properties, $ref (#/definitions/...),
//   anyOf, allOf, oneOf, not, if/then/else.
//
// Anything else is treated as "no constraint". The dataset uses none of the
// unsupported keywords (verified against schema), so this is safe.
...
function typeMatches(value: unknown, expected: string | string[]): boolean {
  ...
  for (const e of list) {
    if (e === "number" && (t === "number" || t === "integer")) return true;
    if (e === t) return true;
  }
  return false;
}
...
function tryValidUri(s: string): boolean {
  try {
    // accept relative URIs too; just must parse
    new URL(s);
    return true;
  } catch {
    // fall back: very permissive — any non-empty string with no whitespace
    return /^[^\s]+$/.test(s);
  }
}
...
function validateNode(
  ...
  if (typeof schema.$ref === "string") {
    ...
    if (target) validateNode(value, target, root, path, errors);
    return;
  }
  ...
  if (Array.isArray((schema as Record<string, unknown>).oneOf)) {
    const subs = (schema as { oneOf: Schema[] }).oneOf;
    let validCount = 0;
    for (const sub of subs) {
      const local: ValidationError[] = [];
      validateNode(value, sub, root, path, local);
      if (local.length === 0) validCount++;
    }
    if (validCount !== 1) {
  ...
  if (isPlainObject((schema as Record<string, unknown>).not)) {
    const local: ValidationError[] = [];
    validateNode(value, (schema as { not: Schema }).not, root, path, local);
    if (local.length === 0) {
  ...
  if (isPlainObject((schema as Record<string, unknown>).if)) {
    const cond = (schema as { if: Schema; then?: Schema; else?: Schema }).if;
    const condErrors: ValidationError[] = [];
    validateNode(value, cond, root, path, condErrors);
    if (condErrors.length === 0 && (schema as { then?: Schema }).then) {
      validateNode(value, (schema as { then: Schema }).then, root, path, errors);
    } else if (condErrors.length > 0 && (schema as { else?: Schema }).else) {
      validateNode(value, (schema as { else: Schema }).else, root, path, errors);
    }
  }
}
