...
export function bouwCsvExportRij({ profiel, commentaar, }: {
    profiel: Profiel;
    commentaar: string;
...
export function profielNaarCsv({ profiel, commentaar, }: {
    profiel: Profiel;
    commentaar: string;
}): string {
...
export function profielUitCsv(csv: string): {
    profiel: Profiel;
    commentaar: string;
} {
    const rijen = parseCsv(csv);
    if (rijen.length < 2)
        throw new Error("CSV bevat geen datarij.");
...
function parseProfielRij(headers: string[], waarden: string[]): {
    profiel: Profiel;
    commentaar: string;
} {
...
export function normaliseerCsvBestandsnaam(naam: string, vandaag: string): string {
    ...
    if (trimmed === "")
        return `${vandaag}-jaakie-export.csv`;
...
function parseProfielWaarde<K extends keyof Profiel>(kolom: K, waarde: string): Profiel[K] {
    ...
    if (typeof voorbeeld === "number")
        return parseNumber(waarde) as Profiel[K];
