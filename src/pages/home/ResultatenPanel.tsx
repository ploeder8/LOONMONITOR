...
export function ResultBandsPanel({ profiel, }: {
    ...
    return (<section style={{ display: "flex", flexDirection: "column", gap: 16, minWidth: 0 }}>
      ...
      <div style={{
        ...
        <button type="button" aria-pressed={auditForce === "all"} onClick={() => setAuditForce(auditForce === "all" ? null : "all")} style={{
      ...
      <AuditOpenProvider force={auditForce}>
        {bands.map((b) => (<ResultBand key={b.id} id={b.id} title={b.title} icon={b.icon}>
            {b.blocks.map((block, i) => (<div key={i}>{block}</div>))}
          </ResultBand>))}
      </AuditOpenProvider>
    </section>);
}
...
function bouwResultaten(p: Profiel): BouwResultaten {
    ...
    if (p.statuut === "bediende") {
        loonbasisBlocks.push(safeRender(() => berekenBediendeLoonbasisVoorProfiel(p, refDatum), ({ barema: r, check: c }) => (<>
            <ResultCard label={`Sectoraal minimum — Schaal ${p.schaal}, Cat ${p.cat}, ${r.effectieveErvaring} jaar`} amountEUR={r.maandloonEUR} helper={[
                ...
                r.geclampt
                    ...
                    : undefined,
            ].filter(Boolean).join(" · ") || undefined} datapunten={[r.datapunt]} highlight/>
            {c.ok ? (<Banner kind="success" title="Brutoloon-check OK">
                {c.vergelijkingsbasis === "deeltijds_omgerekend"
              ...
              </Banner>) : (<Banner kind="error" title="Brutoloon onder sectoraal minimum">
        ...
        loonbasisBlocks.push(safeRender(() => berekenStudentenBaremaVoorProfiel(p, refDatum), (r) => (<ResultCard label={`Studentenbarema — Cat ${p.studentenCat}, ${p.studentLeeftijd} jaar`} amountEUR={r.maandloonEUR} datapunten={[r.datapunt]} highlight/>)));
    }
    bands.push({
        id: "band-loonbasis",
        title: "Loonbasis",
