...
function BaremaInlineCheck({ profiel }: {
    ...
    if (result.kind === "error") {
        return (<Banner kind="warning" title="Barema-check niet beschikbaar">
        {result.message}
      </Banner>);
    }
    return <BaremaInlineCheckCard profiel={profiel} check={result.check}/>;
}
function BaremaInlineCheckCard({ profiel, check, }: {
    profiel: Profiel;
    check: BrutolocheckResult;
}) {
    const isDeeltijds = check.vergelijkingsbasis === "deeltijds_omgerekend";
    return (<div style={baremaInlineStyle(check.ok)}>
      <BaremaInlineHeader profiel={profiel} minimum={check.sectoraalMinimum} effectieveErvaring={check.effectieveErvaring}/>
      {check.ok ? (<span>
          {isDeeltijds
...
function BaremaInlineHeader({ profiel, minimum, effectieveErvaring, }: {
    profiel: Profiel;
    minimum: number;
...
function BaremaClampNote({ ok }: {
    ok: boolean;
}) {
    return (<span style={{ color: ok ? "var(--color-text-muted)" : "#7f1d1d" }}>
...
function ArbeidscontextCard({ profiel, set, setBerekeningsMaand, setBerekeningsJaar, }: {
    profiel: Profiel;
    set: ProfielSetter;
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {profiel.statuut === "bediende" && (<div className="grid grid-cols-3 gap-3">
            ...
            <FormField label="Ervaring">
              <NumeriekeInput className={inputClass} min={0} max={60} value={profiel.ervaringJaren} modus="int" onValueChange={(waarde) => set("ervaringJaren", waarde)}/>
            </FormField>
          </div>)}
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <FormField label={<>Werkdagen <HelpTooltip text="Vooringevuld op basis van weekdagen in de gekozen maand. Aanpasbaar voor feestdagen, verlof of afwijkende prestaties."/></>}>
            <NumeriekeInput className={inputClass} min={0} max={31} value={profiel.arbeidsdagenPerMaand} modus="int" onValueChange={(waarde) => set("arbeidsdagenPerMaand", waarde)}/>
          </FormField>
        </div>

        {profiel.statuut === "bediende" && (<FormField label="Tewerkstelling (%)">
            <NumeriekeInput className={inputClass} step="1" min={1} max={100} value={tewerkstellingsbreukNaarPercentage(profiel.tewerkstellingsbreuk)} onValueChange={(waarde) => set("tewerkstellingsbreuk", percentageNaarTewerkstellingsbreuk(waarde))}/>
          </FormField>)}
      </div>
    </CockpitCard>);
}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        {profiel.statuut === "bediende" && profiel.berekeningsRichting === "bruto_naar_netto" ? (<FormField label="Brutoloon (€)">
            ...
            <FormField label="Gewenst nettoloon (€)">
              <NumeriekeInput className={inputClass} step="0.01" value={profiel.doelNettoloon} onValueChange={(waarde) => set("doelNettoloon", waarde)}/>
            </FormField>
            <FormField label="Berekend bruto (€)">
              <NumeriekeInput className={inputClass} step="0.01" value={profiel.brutoloon} disabled onValueChange={() => { }}/>
            </FormField>
