...
export function MobiliteitPaneel({ profiel, set, setAlleWoonwerk, }: {
    ...
    return (<CockpitCard title="Woon-werk verkeer" icon={<Bike size={16}/>}>
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        <VervoersmiddelRij label="Fiets" icon={<Bike size={15}/>} actief={profiel.woonwerkFiets} onChange={toggleFiets}>
          {profiel.woonwerkFiets && (<div style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <NumeriekeInput className={inputClass} min={0} value={profiel.kmPerDag} onValueChange={(waarde) => set("kmPerDag", waarde)} style={{ width: 70, textAlign: "right" }}/>
        ...
        <VervoersmiddelRij label="Privéwagen" icon={<Car size={15}/>} actief={profiel.woonwerkPrivewagen} onChange={togglePrivewagen}>
          {profiel.woonwerkPrivewagen && (<>
              <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                <NumeriekeInput className={inputClass} min={0} value={profiel.privewagenKm} onValueChange={(waarde) => set("privewagenKm", waarde)} style={{ width: 70, textAlign: "right" }}/>
                <span style={{ fontSize: 11, fontWeight: 600, color: "var(--color-text-muted)" }}>km/dag (enkele rit)</span>
              </div>
            </>)}
        </VervoersmiddelRij>
        {profiel.woonwerkPrivewagen && (<div className="flex items-center gap-3" style={{ paddingLeft: 32, marginTop: -4 }}>
        ...
        <VervoersmiddelRij label="Bus / tram / metro" icon={<Bus size={15}/>} actief={profiel.woonwerkBusTramMetro} onChange={(v) => set("woonwerkBusTramMetro", v)}>
          {profiel.woonwerkBusTramMetro && (<>
              <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                <NumeriekeInput className={inputClass} min={0} value={profiel.busTramMetroKm} onValueChange={(waarde) => set("busTramMetroKm", waarde)} style={{ width: 70, textAlign: "right" }}/>
                <span style={{ fontSize: 11, fontWeight: 600, color: "var(--color-text-muted)" }}>km/dag (enkele rit)</span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                <NumeriekeInput className={inputClass} step="0.01" min={0} value={profiel.busTramMetroPrijs} onValueChange={(waarde) => set("busTramMetroPrijs", waarde)} style={{ width: 70, textAlign: "right" }}/>
        ...
        <VervoersmiddelRij label="Trein" icon={<Train size={15}/>} actief={profiel.woonwerkTrein} onChange={(v) => set("woonwerkTrein", v)}>
          {profiel.woonwerkTrein && (<div style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <NumeriekeInput className={inputClass} min={0} value={profiel.treinKm} onValueChange={(waarde) => set("treinKm", waarde)} style={{ width: 70, textAlign: "right" }}/>
              <span style={{ fontSize: 11, fontWeight: 600, color: "var(--color-text-muted)" }}>km/dag (enkele rit)</span>
            </div>)}
        </VervoersmiddelRij>
        ...
        {profiel.woonwerkBedrijfswagen && (<div style={{ paddingLeft: 32, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            ...
            {profiel.bedrijfswagenBrandstof !== "elektriciteit" && (<FormField label="CO₂-waarde">
    ...
    return (<div>
      <label style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <input type="checkbox" checked={actief} onChange={(e) => onChange(e.target.checked)} style={{ accentColor: "var(--color-primary)", width: 16, height: 16 }}/>
