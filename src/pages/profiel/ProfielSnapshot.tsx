...
function periodeLabel(profiel: Profiel): string {
  return `${MAANDEN[profiel.berekeningsMaand] ?? profiel.berekeningsMaand} ${profiel.berekeningsJaar}`;
}
...
export function ProfielSnapshot({ profiel }: { profiel: Profiel }) {
  ...
  return (
    ...
    >
      ...
      >
        <SnapshotItem icon={<CalendarDays size={14} />} label="Periode" value={periodeLabel(profiel)} />
        <SnapshotItem icon={<Euro size={14} />} label="Loon" value={geldLabel(profiel)} />
        <SnapshotItem
          icon={<BriefcaseBusiness size={14} />}
          label="Context"
          value={`${profiel.statuut} · ${Math.round(profiel.tewerkstellingsbreuk * 100)}%`}
        />
        <SnapshotItem icon={<UsersRound size={14} />} label="Fiscaal" value={gezin} />
        <SnapshotItem
          icon={<ReceiptText size={14} />}
          label="Voordelen"
          value={profiel.maaltijdchequesActief ? "maaltijdcheques actief" : "geen maaltijdcheques"}
        />
