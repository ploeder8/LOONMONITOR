...
export function HomePage() {
  ...
  return (
    ...
    >
      ...
      {/* Werknemer overzicht modal */}
      {toonOverzicht && createPortal(
        <div
          className="print-modal-overlay werknemer-overzicht-modal-overlay"
          style={{
            position: "fixed",
            inset: 0,
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setToonOverzicht(false);
          }}
        >
          <div style={{ maxWidth: 900, width: "100%", position: "relative" }}>
            <button
              type="button"
              onClick={() => setToonOverzicht(false)}
              style={{
                position: "absolute",
                top: -16,
                right: -16,
